...
class HP1000Driver(weewx.drivers.AbstractDevice):
    ...
    def connectToWeatherStation(self, reconnect=False):
...
#                    loginf( 'IP Address = %s' % self.ws_IP_address)
                ...
                # See if this is not a reconnect
                if not reconnect:
        ...
        while True:
            ...
            if self.internal_test_mode:
                ...
                if self.iteration_count > self.max_iterations:
                    loginf("glp - exit wth iteration_count = {0}".format(self.iteration_count))
                    sys.exit()
    ...
    def getHistoryData( self, year, record_count, starting_record):
        ...
        try:
            ...
            # Pipe broken - Weather staiton has shut down the link as nothing happening for too long
            self.ws_socket = None
            self.connectToWeatherStation(True)
        try:
